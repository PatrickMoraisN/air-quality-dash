...
describe('getNeighborhoodsFilteredPaginated', () => {
  ...
  it('should return paginated and filtered neighborhoods', async () => {
    ...
    const page = 1
    const limit = 2
    const result = await airQualityAPI.getNeighborhoodsFilteredPaginated(qualities, page, limit)

    expect(api.get).toHaveBeenCalledWith(`${airQualityApiRoutes.index}?_sort=id&_order=asc`)
